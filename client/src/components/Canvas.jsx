...
function Canvas() {
  ...
  useEffect(() => {
    ...
    socket.on("event", (event) => {
      ...
      drawingArea.current.dispatchEvent(newEvent);
    });

    socket.on("clear", () => {
      setContext((prevContext) => {
        prevContext.putImageData(
          prevContext.createImageData(
            prevContext.canvas.width,
            prevContext.canvas.height
          ),
          0,
          0
        );
  ...
  const onMouseDown = (e) => {
    ...
    if (user === currentDrawer) sendEvent(e, x, y);
  };
