...
function Arena() {
	...
	useEffect(() => {
		...
		if (timer === 0) {
			...
			if (owner) socket.emit("timeout", roomId, guessed);
		}
	...
	const onGuessSumbit = (e) => {
		if (e.key === "Enter") {
			if (checkGuess(guess)) {
				socket.emit("guessed", roomId, user, timer);
				setGuess("");
				return;
			}

			socket.emit("message", roomId, user, guess);
			setGuess("");
			return;
		}
		return;
	...
	const onWordSelect = (e) => {
		socket.emit("wordChoice", roomId, e.target.innerText);
		// setSelected(true);
	};
				...
				<div className="flex flex-col items-center gap-2 basis-8/12 h-full relative">
					...
					{declareWinner ? (
						<div className="flex flex-col gap-2 absolute left-0 top-0 h-3/4 w-full justify-center items-center bg-black self-center">
							<p className="text-5xl text-amber-400">
								Final Scores
							</p>
							<div className="flex flex-col gap-2 items-start">
								{sortByScore(active, leaderboard).map(
									(member, idx) => (
										<p
											key={idx}
							...
							{owner ? (
								<div
									className="btn btn-sm mt-4 game"
									onClick={onNewGame}
								>
									New Game
								</div>
